...
if(isset($_POST[submit]))
{
	...
	{
		echo "<script>alert('Books and startup Idea Stored suuccessfully ');</script>";
	}
	else
	{
		...
		if($qsql = mysqli_query($con,$sql))
		{
			echo "<script>alert('Startup Idea inserted successfully...');</script>";
		}
		else
		{
			echo mysqli_error($con);
...
<div class="wrapper col2">
  <div id="breadcrumb">
    <ul>
      <li class="first">Share Your Used Books at half price</li></ul>
  </div>
</div>
...
if(isset($_POST[submit]))
{
	if(mysqli_num_rows($qsqlappointment) >= 1)
	{		
			echo "<h2>Books and startup program insrted successfully".  " .. </h2>";
	}
	else
	{
		if(isset($_SESSION[patientid]))
		{
			echo "<h2>startup program sent succesfully </h2>";
			echo "<p>Startup Idea is in pending process. Kinldy check the startup status. </p>";
			echo "<p> <a href='viewappointment.php'>View Appointment record</a>. </p>";			
		}
		else
		{
			echo "<h2>startup program sent succesfully </h2>";
			echo "<p>Startup Idea is in pending process. Please wait for confirmation message.. </p>";
...
?>
        ...
        <tr>
          <!--<td><strong>Enter Your Startup Idea</strong></td>
          <td><input type="text" min="<?php echo date("Y-m-d"); ?>" name="appointmentdate" id="appointmentdate" ></td>
        </tr>-->
        <tr>
          <td>Enter Available Time For meeting</td>
